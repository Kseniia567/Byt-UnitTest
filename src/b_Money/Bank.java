...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			accountlist.put(accountid, new Account(accountid, this.currency)); //java.lang.NullPointerException: Cannot invoke "b_Money.Account.deposit(b_Money.Money)" because "account" is null
			//changed "get" to "put" and create new Account record
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) { //Was error in condition. Added "!" to negotiate condition
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			account.withdraw(money); //"Deposit" instead of "Withdraw". Replaced by Withdraw
		}
	}
	...
	public Integer getBalance(String accountid) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			return accountlist.get(accountid).getBalance().getAmount().intValue();
		}
	}
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(fromaccount) || !accountlist.containsKey(toaccount)) {
			...
			transfer(fromaccount, this, toaccount, amount); //second from account should be replaced by toaccount
		}
